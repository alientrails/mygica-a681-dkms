...
{
	struct dvb_usb_device *d = adap->dev;
	static struct mndmd_config config = {
		.tuner_address = 0x60,
	};
	...
	/* Attach frontend */
	/*
	 * mndmd_attach is linked into mygica_a681.ko (see the DKMS Makefile),
	 * so calling it directly is required.  dvb_attach() performs a runtime
	 * symbol lookup intended for a separate demodulator module; that lookup
	 * cannot resolve a symbol from the module currently being initialized.
	 */
	adap->fe_adap[0].fe = mndmd_attach(&config, &d->i2c_adap);
	if (!adap->fe_adap[0].fe)
		return -EIO;
...
{
	return dvb_usb_device_init(intf, &cxusb_mygica_a681_properties,
				   THIS_MODULE, NULL, adapter_nr) ? -ENODEV : 0;
}
...
static struct dvb_usb_device_properties cxusb_mygica_a681_properties = {
	...
	.adapter = {
		...
		.fe = {{
			...
			/*
			 * The A681 uses an MxL603 tuner controlled by mn88436.c.
			 * Do not attach the MAX2165 handler used by the unrelated
			 * MyGica D689 model.
			 */
			.tuner_attach     = NULL,
	...
	.devices = {
		{
			"MyGica / Geniatech A681 ATSC/ClearQAM",
			{ NULL },
